...
const AUTH_CACHE_TTL_MS = Math.max(
  30000,
  Number(process.env.AUTH_CACHE_TTL_MS || 10 * 60 * 1000),
);
...
const buildUserFromDecodedToken = (decoded) => {
  const isMasterAdmin = isMasterAdminUser(decoded);
  const effectiveRole = isMasterAdmin
    ? "Super Admin"
    : String(decoded?.role || "").trim();
  ...
  return {
    ...
    is_super_admin:
      isMasterAdmin ||
      ["super admin", "superadmin"].includes(effectiveRole.toLowerCase()),
    permissions,
    ...permissionFlags,
    _degraded_auth: true,
  };
...
const getDefaultRolePermissions = (roleNameRaw, employeeIdRaw = null) => {
  const roleName = String(roleNameRaw || "")
    .trim()
    .toLowerCase();

  if (isMasterAdminUser({ employee_id: employeeIdRaw })) {
...
const authMiddleware = async (req, res, next) => {
  ...
  try {
    ...
    if (!authSecret) {
      console.error(
        "Auth Middleware Error: JWT_SECRET/SESSION_SECRET is missing in environment",
      );
      return res
        .status(500)
    ...
    const rolePermissions =
      ...
      typeof userData.role_permissions === "object" &&
      !Array.isArray(userData.role_permissions)
        ? userData.role_permissions
        : {};
    ...
    const isSuperAdmin =
      isMasterAdminUser(userData) ||
      ["super admin", "superadmin"].includes(effectiveRole.toLowerCase());

    const permissions = {
      ...getDefaultRolePermissions(effectiveRole, userData.employee_id),
      ...rolePermissions,
    };
    ...
    console.log(
      `[Auth] User: ${req.user.full_name} (${req.user.id}), Role: ${effectiveRole}, Permissions Keys: ${Object.keys(permissions)}`,
    );
    next();
  } catch (error) {
    ...
    if (transientDbError) {
      ...
      if (cachedUser) {
        ...
        console.warn(
          `[Auth] Using cached auth for user ${cachedUser.id} due to transient DB error`,
        );
        return next();
      }
      ...
      if (decoded?.id) {
        ...
        console.warn(
          `[Auth] Using token-fallback auth for user ${decoded.id} due to transient DB error`,
        );
        return next();
      }
