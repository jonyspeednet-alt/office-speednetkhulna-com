...
const getEmployees = async (req, res) => {
    try {
        ...
        const employees = result.rows.map(user => {
            ...
            if (Number(userInfo.id) !== currentUserId) {
...
const getEmployeeById = async (req, res) => {
    try {
        ...
        // Self access always allowed. Cross-user edit requires explicit override permission.
        if (targetUserId !== currentUserId && !canEditAnyProfile(req.user)) {
            return res.status(403).json({ message: 'Unauthorized' });
        }
...
const updateEmployee = async (req, res) => {
    try {
        ...
        // Strict ownership with configurable override policy.
        if (targetUserId !== currentUserId && !hasOverrideEdit) {
            return res.status(403).json({ message: 'Unauthorized' });
        }
