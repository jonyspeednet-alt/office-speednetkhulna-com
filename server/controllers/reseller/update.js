...
const updateReseller = async (req, res) => {
    try {
        ...
        const normalizedJoiningDate = /^\d{4}-\d{2}-\d{2}$/.test(String(joining_date || "").trim()) ? String(joining_date).slice(0, 10) : null;
        ...
        const shouldRefreshProjectedBill = hasBillingImpactingChange && normalizedStatus !== "inactive" && normalizedStatus !== "suspended";
       ...
       FROM resellers WHERE id = $1`,
        ...
        if (hasResellerPartnerTypeColumn() && hasExplicitPartnerType && normalizedPartnerType) {
            await pool.query(`UPDATE resellers SET partner_type = $1 WHERE id = $2`, [normalizedPartnerType, id]);
        }
        ...
        if (req.body.channel_user_count !== undefined) {
            const cuc = Math.max(0, parseInt(req.body.channel_user_count || 0, 10) || 0);
            await pool.query(`UPDATE resellers SET channel_user_count = $1 WHERE id = $2`, [cuc, id]).catch(() => {});
        }
