...
const { getActor, getReqMeta, logResellerFinancialChange } = require("../../utilities/resellerFinancialAudit");
...
const changeResellerRate = async (req, res) => {
    try {
        ...
        const { effective_date, note, rate_iig, rate_bdix, rate_ggc, rate_fna, rate_cdn, rate_bcdn, rate_nttn } = req.body;
        ...
        const hasAnyRate = [rate_iig, rate_bdix, rate_ggc, rate_fna, rate_cdn, rate_bcdn, rate_nttn].some((v) => v !== undefined && v !== null && v !== '');
        if (!hasAnyRate) {
            return res.status(400).json({ message: "At least one rate field is required" });
        }

        const beforeResult = await pool.query(`SELECT rate_iig, rate_bdix, rate_ggc, rate_fna, rate_cdn, rate_bcdn, rate_nttn FROM resellers WHERE id = $1`, [id]);
        ...
        try {
         ...
         RETURNING id`,
            ...
            for (const [bwType, newRate] of Object.entries(bwTypeMap)) {
                ...
                if (newRate !== prevRate) {
                    await client.query(`DELETE FROM reseller_rate_history WHERE reseller_id=$1 AND bw_type=$2 AND effective_date=$3::date`, [id, bwType, effective_date]);
                    await client.query(`INSERT INTO reseller_rate_history (reseller_id, bw_type, rate, effective_date) VALUES ($1, $2, $3, $4::date)`, [id, bwType, newRate, effective_date]);
                }
            }
