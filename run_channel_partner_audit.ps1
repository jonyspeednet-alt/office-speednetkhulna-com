...
$localRunner = Join-Path $root 'run_channel_partner_audit.js'
$remoteRunner = '/tmp/run_channel_partner_audit.js'
& $pscp -batch -P $Port -pw $Password $localRunner "${Server}:$remoteRunner" | Out-Null
if ($LASTEXITCODE -ne 0) { throw 'Failed to upload audit runner' }
$ErrorActionPreference = 'Continue'
$output = & $plink -batch -ssh -P $Port -pw $Password $Server "cp $remoteRunner $RemoteRoot/server/run_channel_partner_audit.js && cd $RemoteRoot/server && node run_channel_partner_audit.js && rm -f run_channel_partner_audit.js $remoteRunner" 2>&1
