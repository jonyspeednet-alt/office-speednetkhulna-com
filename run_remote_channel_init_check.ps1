...
$localRunner = Join-Path $root 'run_remote_channel_init_check.js'
$remoteRunner = '/tmp/run_remote_channel_init_check.js'
& $pscp -batch -P $Port -pw $Password $localRunner "${Server}:$remoteRunner" | Out-Null
if ($LASTEXITCODE -ne 0) { throw 'Failed to upload init check runner' }
$ErrorActionPreference = 'Continue'
$output = & $plink -batch -ssh -P $Port -pw $Password $Server "cp $remoteRunner $RemoteRoot/server/run_remote_channel_init_check.js && cd $RemoteRoot/server && node run_remote_channel_init_check.js && rm -f run_remote_channel_init_check.js $remoteRunner" 2>&1
