...
const Sidebar = () => {
  ...
  const menuData = useMemo(() => {
    ...
    const currentRole = String(userRole || user?.role || '').trim().toLowerCase();
    const isSuperAdmin = currentRole === 'super admin' || currentRole === 'superadmin';
    const assetMenuPresent = Object.values(normalized).some((menus) =>
      Array.isArray(menus) && menus.some((menu) => getCleanLink(menu.link) === '/asset-management')
    );

    if (isSuperAdmin && !assetMenuPresent) {
      if (!normalized.Administration) normalized.Administration = [];
      normalized.Administration.push({
        id: 1013,
    ...
    return normalized;
  }, [rawMenuData, user?.role, userRole]);
