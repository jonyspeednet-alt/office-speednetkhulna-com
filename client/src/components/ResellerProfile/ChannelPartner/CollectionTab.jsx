...
import { money, getDhakaDateYmd } from '../../../utils/formatters';
...
const CollectionTab = ({
    ...
    const totalCollection = cpUserPayments.reduce((s, p) => s + Number(p.amount_paid || 0), 0);
    const totalDue = cpUserPayments.reduce((s, p) => s + Number(p.amount_due || 0), 0);
    const totalPending = cpUserPayments.reduce((s, p) => s + (Number(p.amount_due || 0) - Number(p.amount_paid || 0)), 0);
    const collectionRate = Math.round((cpUserPayments.filter(p => Number(p.amount_paid) > 0).length / (cpUserPayments.length || 1)) * 100);
        ...
        <div className="p-3">
                    ...
                    <div className="card border-0 shadow-sm rounded-4 h-100 overflow-hidden position-relative" style={{ background: 'linear-gradient(135deg, #4318ff 0%, #7c58ff 100%)' }}>
                        <div className="card-body p-3 text-white">
                            <div className="d-flex justify-content-between align-items-center mb-2">
                                <div className="rounded-circle bg-white bg-opacity-20 p-2" style={{ width: 35, height: 35, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <div className="card border-0 shadow-sm rounded-4 h-100 overflow-hidden" style={{ background: 'linear-gradient(135deg, #05cd99 0%, #00a38d 100%)' }}>
                        <div className="card-body p-3 text-white">
                            ...
                            <div className="progress mt-2" style={{ height: 4, backgroundColor: 'rgba(255,255,255,0.2)' }}>
                                <div className="progress-bar bg-white" style={{ width: `${Math.min(100, (totalCollection / (totalDue || 1)) * 100)}%` }}></div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="d-flex justify-content-between align-items-center mb-3">
                ...
                <div className="d-flex gap-2">
                    <button className="btn btn-sm btn-info text-white rounded-pill px-3 shadow-sm" onClick={onImportClick}>
                        <i className="fas fa-file-excel me-1" /> Excel Import
                    </button>
                    <button className="btn btn-sm btn-primary rounded-pill px-3 shadow-sm" onClick={onInitPayments}>
                    ...
                    <tbody>
                        {cpUserPayments.length === 0 ? (
                            ...
                            <tr key={p.id}>
                                ...
                                <td>
                                    <span className={`badge ${Number(p.amount_paid) > 0 ? 'bg-success' : 'bg-warning'} bg-opacity-10 text-dark border`}>
                                        {Number(p.amount_paid) > 0 ? 'Paid' : 'Unpaid'}
                                    </span>
                                </td>
                                <td>
                                    {Number(p.amount_paid) === 0 && (
                                        <button
                                            ...
                                            onClick={() => onRecordPayment(p.user_id, Number(p.amount_due))}
                                        >
                                            Full Pay
                                        </button>
                                    )}
