...
const ResellerStatusNoc = () => {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const data = await getResellerStatusNoc({ status: statusView });
  ...
  return (
    <div className="container-fluid py-3 reseller-page">
      ...
      <div className="card border-0 shadow-sm rounded-4">
        ...
        <div className="px-3 pt-3 d-flex gap-2 flex-wrap">
          <button
            type="button"
            className={`btn ${statusView === 'active' ? 'btn-dark' : 'btn-outline-dark'}`}
          ...
          <button
            type="button"
            className={`btn ${statusView === 'inactive' ? 'btn-danger' : 'btn-outline-danger'}`}
          ...
          <table className="table align-middle mb-0">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan="11" className="text-center py-5 text-muted">{statusView === 'inactive' ? 'কোনো inactive reseller পাওয়া যায়নি' : 'কোনো রিসেলার পাওয়া যায়নি'}</td></tr>
              ) : rows.map((r) => (
                <tr key={r.id}>
                  <td>
