...
export const useChannelPartner = (profileId, isChannel) => {
    ...
    const handleBulkFullPaid = async () => {
        ...
        await bulkRecordPayments(profileId, cpMonth, unpaid.map(p => ({
            user_id: p.user_id,
    ...
    const handleCommissionPayment = async (e) => {
        ...
        setCommPayForm({ amount: '', payment_date: getDhakaDateYmd(), payment_method: 'Cash', reference_no: '', note: '' });
        loadChannelData();
    };
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            window.alert(err?.response?.data?.message || 'ইম্পোর্ট করতে সমস্যা হয়েছে।');
        } finally {
            setImporting(false);
        }
    };
